    ...
    <div className="w-full py-3">
      <div className="flex gap-3 items-center mb-1">
        <Link to={"/profile/" + reply?.userId?._id}>
          <img
            src={reply?.userId?.profileImg ?? NoProfile}
      ...
      <div className="flex items-end justify-end pb-2">
        {loading ? (
          ...
          <CustomButton
            title="Submit"
            type="submit"
            containerStyles="bg-blueSecondary  text-white py-1 px-3 rounded-full font-semibold text-sm"
          />
        )}
      </div>
    </form>
...
const PostCard = ({ post, user }) => {
  ...
  return (
    <div className="mb-2 bg-primary p-4 rounded-xl">
      <div className="flex gap-3 items-center mb-2">
        <Link to={"/profile/" + post?.userId?._id}>
          <img
            ...
            className="h-13 w-14  object-cover rounded-full"
          />
        </Link>
        ...
        <div className="w-full flex justify-between">
          <div className="">
            <Link to={"/profile/" + post?.userId?._id}>
              <p className="font-medium text-2xl text-ascent-1">
                {post?.userId?.firstName} {post?.userId?.lastName}
              </p>
            </Link>
            <span className="text-ascent-2">{post?.userId?.location}</span>
            <span className="flex md:hidden text-ascent-2 text-xs ">
      ...
      <div>
        <p className="text-ascent-2 flex pl-4">
          {showAll === post?._id
            ? post?.description
          ...
          <div className="p-3">
            <Slider {...sliderSettings}>
              {post.files.map((image, index) => (
                <div key={index} className="w-full  h-[350px] outline-none ">
                  <img
                    src={image}
              ...
              <div className="w-full py-2" key={comment?._id}>
                <div className="flex gap-3 items-center mb-1">
                  <Link to={"/profile/" + comment?.userId?._id}>
                    <img
                      src={comment?.userId?.profileImg ?? NoProfile}
